...
class ColorMap:
    ...
    def build_rotation_matrices(self):
        # Potentail cycles: Red, Green, Blue, Yellow - [0,1,2,3]
        ...
        if top<4:
            if left<4:
                ...
                # the letter is normal if the left rotation is monotnoic (except from 3 to 0) [0,1,2,3] and its rotations
                # the letter is upside down if the left rotation matric is reversed (except from 0 to 3) [3,2,1,0] and its rotations
                ...
                self.up_roation=deque([left, (top+1+2)%4, second, (top+1)%4])
        else:
    ...
    def rotate_left(self):
        ...
        self.up_roation=deque([self.left_rotation[0], self.up_roation[1], self.left_rotation[2],self.up_roation[3]])
    ...
    def rotate_up(self):
        self.up_roation.rotate(-1)
        self.left_rotation=deque([self.up_roation[0], self.left_rotation[1], self.up_roation[2],self.left_rotation[3]])
