...
def setup():
	...
	add_table_collision(scene, robot.get_planning_frame())
...
def add_cubes_for_collision_but_not(id_not_to_add, cubes, scene, planning_frame):
	for cube in cubes:
		if cube.id is not id_not_to_add:
			ps = PoseStamped()
			ps.header.frame_id = planning_frame
			ps.pose = cube.pose
			scene.add_box('cube_{0}'.format(cube.id), ps, (0.045, 0.045, 0.045))

def remove_cube_collisions(scene, planning_frame):
	scene.remove_world_object()
	add_table_collision(scene, planning_frame)


def add_table_collision(scene, planning_frame):
	# add table as collision object
